...
for k in {0..2}; do           # counts statistics
	for j in {0..0}; do       # counts indices variables
		...
		for l in {0..1}; do   # counts experiments
			# projections
			proj_bc_file=$path_indices_ens_tas/${TAS_indices[j]}_icclim-4-2-3_KNMI_ens-multiModel-${statistics[k]}-${experiment[l]}-EUR-44-SMHI-DBS43-EOBS10-bcref-1981-2010_yr_20060101-20991231.nc
			echo 
			echo "Going for file:"
			echo $proj_bc_file
			echo
		...
		# historical
		hist_bc_file=$path_indices_ens_tas/${TAS_indices[j]}_icclim-4-2-3_KNMI_ens-multiModel-${statistics[k]}-historical-EUR-44-SMHI-DBS43-EOBS10-bcref-1981-2010_yr_19700101-20051231.nc
		echo 
		echo "Going for file:"
		echo $hist_bc_file
		echo
		...
		# projections
		for l in {0..1}; do   # counts experiments
			proj_no_bc_file=$path_indices_ens_tas_nobias/${TAS_indices[j]}_icclim-4-2-3_KNMI_ens-multiModel-${statistics[k]}-${experiment[l]}-EUR-44_yr_20060101-20991231.nc
			echo 
			echo "Going for file:"
		...
		# historical
		hist_no_bc_file=$path_indices_ens_tas_nobias/${TAS_indices[j]}_icclim-4-2-3_KNMI_ens-multiModel-${statistics[k]}-historical-EUR-44_yr_19700101-20051231.nc
		echo 
		echo "Going for file:"
