...
# -------------------
PRECIP_indices=('rx1day' 'r95p' 'r20mm' 'r1mm' 'r10mm' 'prcptot' 'cdd' 'cwd')
...
path_indices_ens_precip=$indices_path/BC/ens_multiModel_pr
# Non-Bias corrected files
path_indices_ens_precip_nobias=$indices_path/No_BC/ens_multiModel_pr_no_bc

# =========================================================================


for k in {0..2}; do           # counts statistics
	for j in {0..7}; do       # counts indices variables


		# BIAS CORRECTED
		# ===================================================================================================================================================================================
		cd $path_indices_ens_precip

		for l in {0..1}; do   # counts experiments
			# projections
			proj_bc_file=$path_indices_ens_precip/${PRECIP_indices[j]}_icclim-4-2-3_KNMI_ens-multiModel-${statistics[k]}-${experiment[l]}-EUR-44-SMHI-DBS43-EOBS10-bcref-1981-2010_yr_20060101-20991231.nc
			echo 
			echo "Going for file:"
			echo $proj_bc_file
			echo
		...
		# historical
		hist_bc_file=$path_indices_ens_precip/${PRECIP_indices[j]}_icclim-4-2-3_KNMI_ens-multiModel-${statistics[k]}-historical-EUR-44-SMHI-DBS43-EOBS10-bcref-1981-2010_yr_19700101-20051231.nc
		echo 
		echo "Going for file:"
		echo $hist_bc_file
		echo
		...
		# Non BIAS CORRECTED
		# ===================================================================================================================================================================================
		cd $path_indices_ens_precip_nobias

		# projections
		for l in {0..1}; do   # counts experiments
			proj_no_bc_file=$path_indices_ens_precip_nobias/${PRECIP_indices[j]}_icclim-4-2-3_KNMI_ens-multiModel-${statistics[k]}-${experiment[l]}-EUR-44_yr_20060101-20991231.nc
			echo 
			echo "Going for file:"
		...
		# historical
		hist_no_bc_file=$path_indices_ens_precip_nobias/${PRECIP_indices[j]}_icclim-4-2-3_KNMI_ens-multiModel-${statistics[k]}-historical-EUR-44_yr_19700101-20051231.nc
		echo 
		echo "Going for file:"
